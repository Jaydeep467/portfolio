...
function App() {
  ...
  return (
    <div className="font-sans text-white bg-[#080810] scroll-smooth">
      <nav className="fixed top-0 left-0 w-full z-[9999] bg-[#080810] border-b border-white/5" style={{ isolation: 'isolate' }}>
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <button
            ...
            onClick={() => {
              console.log('clicked, menuOpen was:', menuOpen);
              setMenuOpen((o) => !o);
            }}
        ...
        {/* Dropdown - now fully solid */}
        <div className={`md:hidden transition-all duration-300 overflow-hidden ${
          menuOpen ? "max-h-screen" : "max-h-0"
        } bg-[#080810]`} style={{ position: 'relative', zIndex: 9999, background: '#080810' }}>
          {NAV_LINKS.map(({ label, href }) => {
            const id = href.replace("#", "");
            return (
              <a key={href} href={href} onClick={() => setMenuOpen(false)}
                className={`block px-8 py-5 text-base border-b border-white/5 transition-all duration-200 ${
                  active === id ? "text-[#4f9eff]" : "text-white/60 hover:text-white hover:bg-white/5"
                }`}
